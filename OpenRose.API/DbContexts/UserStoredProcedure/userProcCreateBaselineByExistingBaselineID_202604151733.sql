...
DECLARE @SourceBaselineItemzHierarchyId hierarchyid
	...
	IF @TempBaselineItemzTypeNumberOfRows > 0
		...
		BEGIN
			...
			SELECT blitz.ItemzId, blitz.Name, blitz.Status, blitz.Priority, blitz.Description, blitz.CreatedBy, blitz.CreatedDate, blitz.Severity ,Tags , @CurrentBaselineItemzTypeID, blitz.isIncluded
		...
		SELECT @NewBaselineID, 'Baseline', @NextAvailableBaselineHierarchyIdUnderProject.ToString(),1,@Name,ISNULL(bih_source.EstimationUnit,''),ISNULL(bih_source.OwnEstimation,0),ISNULL(bih_source.RolledUpEstimation,0)
		FROM BaselineItemzHierarchy bih_source
		WHERE bih_source.Id = @BaselineId
		...
		SELECT bit2.id AS Id
			...
			, bih.isIncluded
			, bih.Name
			, ISNULL(bih.EstimationUnit,'') AS EstimationUnit
			, ISNULL(bih.OwnEstimation,0) AS OwnEstimation
			, ISNULL(bih.RolledUpEstimation,0) AS RolledUpEstimation
		...
		INSERT INTO BaselineItemzJoinItemzTrace(BaselineFromItemzId, BaselineToItemzId, BaselineId, TraceLabel)
		SELECT 
