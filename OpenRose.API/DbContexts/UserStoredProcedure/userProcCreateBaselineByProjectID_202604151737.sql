...
DECLARE @NextAvailableBaselineHierarchyIdUnderProject hierarchyid
	...
	WHERE Id = @ProjectId 
		...
		INSERT into [dbo].[BaselineItemzHierarchy] (Id, RecordType,BaselineItemzHierarchyId,isIncluded,Name,EstimationUnit,OwnEstimation,RolledUpEstimation)
					SELECT @NewBaselineID, 'Baseline', @NewBaselineItemzHierarchyIDString,1,@Name,ISNULL(ith_source.EstimationUnit,''),ISNULL(ith_source.OwnEstimation,0),ISNULL(ith_source.RolledUpEstimation,0)
					FROM ItemzHierarchy ith_source
					WHERE ith_source.Id = @ProjectId
	...
	IF @TempBaselineItemzTypeNumberOfRows > 0
		...
		BEGIN
			...
			SELECT itz.Id, itz.Name, itz.Status, itz.Priority, itz.Description, itz.CreatedBy, itz.CreatedDate, itz.Severity, itz.Tags, @CurrentBaselineItemzTypeID, 1 as isIncluded
		...
		SELECT bi1.Id AS BaselineItemzFromIdTrace , bi2.id AS BaselineItemzToIdTrace , @NewBaselineID AS NewBaselineID , ijit.TraceLabel
