...
DECLARE @NextAvailableBaselineHierarchyIdUnderProject hierarchyid
	...
	WHERE Id = @ProjectId 
		...
		-- WE WILL THEM MOVE IT UNDER PROJECT. 
			...
			DECLARE @ItemzTypeEstimationUnit nvarchar(max)
			DECLARE @ItemzTypeRolledUpEstimation decimal(18,2)

			SET @NewBaselineItemzHierarchyIDString = (select  CONCAT('/', floor(99999 * RAND(convert(varbinary, newid()))), '/'))

			SELECT @ItemzTypeEstimationUnit = ISNULL(EstimationUnit, ''),
				   @ItemzTypeRolledUpEstimation = ISNULL(RolledUpEstimation, 0)
			FROM ItemzHierarchy
			WHERE Id = @ItemzTypeId

			INSERT into [dbo].[BaselineItemzHierarchy] (Id, RecordType,BaselineItemzHierarchyId,isIncluded,Name,EstimationUnit,OwnEstimation,RolledUpEstimation)
							VALUES (@NewBaselineID, 'Baseline', @NewBaselineItemzHierarchyIDString,1,@Name,@ItemzTypeEstimationUnit,0,@ItemzTypeRolledUpEstimation)
		...
		INSERT into [dbo].[BaselineItemzHierarchy] (Id,RecordType,BaselineItemzHierarchyId,SourceItemzHierarchyId,isIncluded,Name,EstimationUnit,OwnEstimation,RolledUpEstimation)
		SELECT baselineIT.id as Id
			...
			, 1 as isIncluded
			,ith.Name
			, ISNULL(ith.EstimationUnit,'') AS EstimationUnit
			, ISNULL(ith.OwnEstimation,0) AS OwnEstimation
			, ISNULL(ith.RolledUpEstimation,0) AS RolledUpEstimation
	...
	IF @TempBaselineItemzTypeNumberOfRows > 0
		...
		BEGIN
			...
			INSERT into [dbo].[BaselineItemz] (ItemzId, Name, Status, Priority, Description, CreatedBy,CreatedDate,Severity,Tags,IgnoreMeBaselineItemzTypeId,isIncluded)   
			SELECT itz.Id, itz.Name, itz.Status, itz.Priority, itz.Description, itz.CreatedBy, itz.CreatedDate, itz.Severity ,Tags , @CurrentBaselineItemzTypeID, 1 as isIncluded
		...
		INSERT INTO [dbo].[BaselineItemzJoinItemzTrace] (BaselineFromItemzId,BaselineToItemzId,BaselineId,TraceLabel)
		SELECT bi1.Id AS BaselineItemzFromIdTrace
		, bi2.id AS BaselineItemzToIdTrace 
