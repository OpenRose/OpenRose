    ...
    BEGIN
        BEGIN TRY
            ...
            CREATE TABLE #TempRecordHierarchy (
                ...
                EstimationUnit NVARCHAR(128), -- ESTIMATION FIELD 1
                OwnEstimation DECIMAL(18, 2), -- ESTIMATION FIELD 2
                RolledUpEstimation DECIMAL(18, 2) -- ESTIMATION FIELD 3
            );
            ...
            BEGIN
                ...
                INSERT INTO ItemzHierarchy (Id, RecordType, Name, ItemzHierarchyId, EstimationUnit, OwnEstimation, RolledUpEstimation)
                SELECT @NewMainRecordID, 'Project', @NewMainRecordName, @NewMainHierarchyID, 
                       ih.EstimationUnit, ih.OwnEstimation, ih.RolledUpEstimation
                FROM ItemzHierarchy ih
                WHERE ih.Id = @RecordID;
                ...
                INSERT INTO #TempRecordHierarchy (
                    ...
                    EstimationUnit, OwnEstimation, RolledUpEstimation
                )
                SELECT ih.Id,
                       NEWID(),
                       ih.ItemzHierarchyId,
                       ...
                       ih.RecordType,
                       ih.Name,
                       NULL, -- Initially set to NULL for ParentItemzTypeId
                       ih.EstimationUnit, -- ESTIMATION FIELD 1
                       ih.OwnEstimation, -- ESTIMATION FIELD 2
                       ih.RolledUpEstimation -- ESTIMATION FIELD 3
                FROM ItemzHierarchy ih
            ...
            BEGIN
                ...
                INSERT INTO ItemzHierarchy (Id, RecordType, Name, ItemzHierarchyId, EstimationUnit, OwnEstimation, RolledUpEstimation)
                SELECT @NewMainRecordID, 'ItemzType', @NewMainRecordName, @NewMainHierarchyID,
                       ih.EstimationUnit, ih.OwnEstimation, ih.RolledUpEstimation
                FROM ItemzHierarchy ih
                WHERE ih.Id = @RecordID;
            ...
            INSERT INTO ItemzHierarchy WITH (TABLOCKX) (Id, RecordType, Name, ItemzHierarchyId, EstimationUnit, OwnEstimation, RolledUpEstimation)
            SELECT NewRecordID, RecordType, Name, NewHierarchyID, EstimationUnit, OwnEstimation, RolledUpEstimation
            FROM #TempRecordHierarchy
            ...
            SELECT DISTINCT t.FromItemzId, t.ToItemzId, t.TraceLabel,
                COALESCE(n1.NewRecordID, t.FromItemzId) AS NewFromItemzId, 
                COALESCE(n2.NewRecordID, t.ToItemzId) AS NewToItemzId
            ...
            INSERT INTO ItemzJoinItemzTrace WITH (TABLOCKX) (FromItemzId, ToItemzId, TraceLabel)
            SELECT DISTINCT NewFromItemzId, NewToItemzId, TraceLabel
            FROM #TempJoinItemzTrace
            WHERE NewFromItemzId IS NOT NULL AND NewToItemzId IS NOT NULL;
