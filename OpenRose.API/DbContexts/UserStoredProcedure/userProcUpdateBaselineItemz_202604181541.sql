		...
		BEGIN
			...
			IF (@ShouldBeIncluded = 1 OR @ShouldBeIncluded = 0)
				...
				IF (@ShouldBeIncluded = 1 AND @SingleNodeInclusion =  1 )
					BEGIN
						-- PHASE 5: INCLUSION - Single node only
						...
						Where id = @CurrentInputBaselineItemzId
					END 
				ELSE IF (@ShouldBeIncluded = 0)
					BEGIN
						-- PHASE 5: EXCLUSION - Cascade to all descendants
						-- IMPORTANT: Only set RolledUpEstimation = 0 for children, NOT the target record itself
						-- The target record's RolledUpEstimation is preserved for the follow-up deduction process
						
						-- Step 1: Update isIncluded flag for target and all descendants
						UPDATE BaselineItemzHierarchy
						SET isIncluded = @ShouldBeIncluded
						...
						-- Step 2: Set RolledUpEstimation = 0 for ALL DESCENDANTS (children) ONLY
						-- Exclude the target record itself from this update
						UPDATE BaselineItemzHierarchy
						SET RolledUpEstimation = 0
						WHERE BaselineItemzHierarchyId.IsDescendantOf(
							(select BaselineItemzHierarchyiD FROM BaselineItemzHierarchy WHERE id = @CurrentInputBaselineItemzId) ) = 1
						AND id != @CurrentInputBaselineItemzId  -- CRITICAL: Exclude target record itself
					END
				ELSE
					BEGIN
						-- PHASE 5: INCLUSION - All children (cascade inclusion)
						UPDATE BaselineItemzHierarchy
						SET isIncluded = @ShouldBeIncluded
						where BaselineItemzHierarchyId.IsDescendantOf(
							(select BaselineItemzHierarchyiD FROM BaselineItemzHierarchy WHERE id = @CurrentInputBaselineItemzId) ) = 1
